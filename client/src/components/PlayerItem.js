...
class PlayerItem extends React.Component {
        ...
        return (
            <ListGroup.Item variant={this.state.variant}>
                <Row>
                    ...
                    <Col md="auto">
                        <ButtonGroup size='sm'> 
                            {this.canWhisper() && <Button variant={this.props.whisperingTo === this.props.index?"success":"primary"} onClick={() => this.props.openWhisperMenu(this.props.index)}>🗩</Button>}
                            {this.canVote() && <Button variant={this.props.votingFor===this.props.index?"success":"primary"} onClick={() => this.props.handleVote(this.props.index)}>☑</Button>}
                            {this.canVisit() && <Button variant={this.props.visiting===this.props.index?"success":"primary"} onClick={() => this.props.handleVisit(this.props.index)}>◎</Button>}
                        </ButtonGroup>
                    </Col>
                </Row>
            </ListGroup.Item>
    ...
    canVote() {
        ...
        if(this.props.time === 'Night' && this.props.votingFor === null && this.props.canNightVote && this.props.isAlive && this.props.role === undefined && !this.props.isUser) return true;
        return false;
    }

    canWhisper() {
        return this.props.canTalk && this.state.canWhisper && this.props.time === 'Day' && this.props.dayNumber !== 1 && (this.props.whisperingTo === null || this.props.whisperingTo === this.props.index);
    }
