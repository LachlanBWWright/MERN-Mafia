...
class LawmanFaction extends Faction {
    ...
    handleNightVote() { //Called at the end of the night. Forces a random visit for insane members.
        for(let i = 0; i < this.memberList.length; i++) {
            if(this.memberList[i].role.isInsane) { //Selects a random person to visit
                for(let f = 0; f < 100; f++) { //Uses this instead of a while loop just in case some error occurs
                    try{
    ...
    handleNightMessage(message, playerUsername) { //Tells the player that they cannot speak at night.
        for(let i = 0; i < this.memberList.length; i++) {
            if(this.memberList[i].playerUsername == playerUsername) {
                this.io.to(this.memberList[i].socketId).emit('receive-message', 'You cannot speak at night.');
            } 
        }
    }
    ...
    removeMembers() {
        for(let i = 0; i < this.memberList.length; i++) {
            if(!this.memberList[i].isAlive || this.memberList[i].role.name != 'Lawman') {
